...
export const CaregiverRegistrationScreen = ({ onBack, onSuccess, onSignInClick }) => {
  const { t } = useSettings();
      ...
      <div className="mobile-container auth-container" style={{ maxWidth: '520px' }}>
        ...
        <main className="role-main" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', width: '100%' }}>
          ...
          <div className="role-title-section text-center" style={{ marginTop: '0.5rem', marginBottom: '0.5rem' }}>
            <div style={{ width: 52, height: 52, borderRadius: 16, background: 'rgba(234, 88, 12, 0.12)', color: 'var(--color-orange-primary)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 0.5rem auto' }}>
              <Heart size={26} />
            </div>
            <h1 className="role-main-title">Caregiver Registration</h1>
            <p className="role-subtitle">
              Create your caregiver account to support and monitor your patient's speech therapy.
            </p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem', width: '100%' }}>
            ...
            <section className="profile-section-card" style={{ width: '100%', gap: '0.85rem' }}>
              <h3 className="profile-section-title" style={{ color: 'var(--color-orange-primary)' }}>Personal Information</h3>
              ...
              <div className="profile-field-group">
                ...
                {errors.relationship && <span style={{ color: '#DC2626', fontSize: '0.775rem', fontWeight: 600 }}>{errors.relationship}</span>}
              </div>
            </section>

            {/* SECTION 2: ACCOUNT SECURITY */}
            <section className="profile-section-card" style={{ width: '100%', gap: '0.85rem' }}>
              <h3 className="profile-section-title" style={{ color: 'var(--color-orange-primary)' }}>Account</h3>
            ...
            <button
              ...
              style={{ width: '100%', background: 'var(--color-orange-primary)', marginTop: '0.25rem' }}
            >
            ...
            <div style={{ textAlign: 'center', margin: '0.5rem 0' }}>
              ...
              <button
                type="button"
                onClick={onSignInClick || onBack}
                style={{ background: 'none', border: 'none', color: 'var(--color-orange-primary)', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer' }}
              >
                Login
              </button>
            </div>

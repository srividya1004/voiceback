 ...
 * against MongoDB Atlas with MongoMemoryServer fallback.
 */
...
const runTest = async () => {
  try {
    ...
    const uniqueEmail = `test.patient.${Date.now()}@voiceback.org`;
    ...
    const testDoctor = await Doctor.create({
      ...
      licenseNumber: `MD-${ts}`,
      email: `dr.jenkins.test.${ts}@voiceback.org`,
    ...
    const testCaregiver = await Caregiver.create({
      ...
      email: `robert.m.test.${ts}@voiceback.org`
    });
    ...
    console.log('\n--- 6. Testing EmergencySOS Model ---');
    const testEmergencySOS = await EmergencySOS.create({
      patientId: testPatient._id,
      status: 'Resolved',
      location: '12.9716, 77.5946',
      message: 'Test SOS alert'
    });
    console.log(`✅ EmergencySOS created: ID=${testEmergencySOS._id}, Status=${testEmergencySOS.status}`);
    ...
    const testAppointment = await Appointment.create({
      ...
      status: 'Scheduled',
      clinicalNotes: 'Initial post-stroke speech evaluation'
    });
    ...
    await EmergencySOS.findByIdAndDelete(testEmergencySOS._id);
    ...
    console.log('\n🎉 ALL ACTIVE MONGOOSE MODELS TESTED & PASSED SUCCESSFULLY!');
    process.exit(0);
  } catch (error) {
